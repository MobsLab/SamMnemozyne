function figH = sam_sleep_prevspost_bysess(expe, subj, varargin)
%==========================================================================
...
for isubj=1:length(Dir.path)
    ...
    %pre 
    if ss(isubj) 
        [pre_n1, pre_n2, pre_n3, pre_rem, pre_wake] = get_subscoring(substg{isubj}.Epoch_sess{1},tdatpre{isubj}{1});
    ...
    %post
    if ss(isubj) 
        [post_n1, post_n2, post_n3, post_rem, post_wake] = get_subscoring(substg{isubj}.Epoch_sess{2},tdatpost{isubj}{1});
    ...
    predur(isubj) = sum(End(tdatpre{isubj}{1}) - Start(tdatpre{isubj}{1}));
    postdur(isubj) = sum(End(tdatpost{isubj}{1}) - Start(tdatpost{isubj}{1}));
    % same without wake
    ...
    stsess{1}=Start(tdatpre{isubj}{1}); stsess{2}=Start(tdatpost{isubj}{1});
