function wakebycycle_granalyses(Dir, expe, mice_num, varargin)
...
% load data
load([dropbox '/DataSL/Wake/wake_data.mat']);

exp = wake_data.info.exp;
mice_num = wake_data.info.mice_num;
cycleEpoch = wake_data.cycleEpoch;
wakeCycles = wake_data.wakeCycles;
waketotdur = wake_data.waketotdur;
nwake = wake_data.nwake;
ep_len = wake_data.ep_len;
sEpoch = wake_data.sEpoch;
sSession = wake_data.sSession;
...
for iexp=1:numexp %length(expe)
    ...
    for isess=1:nsess(iexp)
        dyn_wake_tmp{isess} = nan(maxsuj,100);  
        ep_len_mean{iexp,isess} = nan(maxsuj,100);
        ep_len_med{iexp,isess} = nan(maxsuj,100);
    end
