...
SleepTimeToRestrict = 1*60*60*1e4; % 1 hours
...
pathfig = [dropbox 'DataSL/Matlab_scripts/Sleep analyses/Spikes/']; % Without dropbox

%set folders
if ~exist([dropbox pathfig],'dir')
    mkdir([dropbox pathfig]);
end
...
for i=1:length(Dir.path)
    ...
    behav{i} = load([Dir.path{i}{1} 'behavResources.mat'],'SessionEpoch','CleanVtsd','AlignedXtsd','AlignedYtsd','FreezeAccEpoch');
    try
        sleep{i} = load([Dir.path{i}{1} 'SleepScoring_OBGamma.mat'],'SWSEpoch','REMEpoch','Sleep');
    catch
        sleep{i} = load([Dir.path{i}{1} 'SleepScoring_Accelero.mat'],'SWSEpoch','REMEpoch','Sleep');
    end
end
...
% Create the last 30 min of sleep - NREM 
for i = 1:length(Dir.path)
    % Pre
...
[p,h,her] = PlotErrorBarN_SL(Pl,...
            'barcolors', [0 0 0], 'barwidth', 0.6, 'newfig', 0, 'colorPoints',1);
        
%% commented because not enough PC for now        
...
% added because not enough PC

[p,h,her] = PlotErrorBarN_SL(Pl,...
            'barcolors', [0 0 0], 'barwidth', 0.6, 'newfig', 0, 'colorPoints',1);
        %% Commented because same as above
% MakeViolinPlot_DB(Pl,Cols,1:5,[],0);
% hold on
% for i=1:2
%     if ~isempty(Pl{i})
