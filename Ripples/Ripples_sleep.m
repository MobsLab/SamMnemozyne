...
% set indices pre/cond/post    
id_Pre = cell(1,length(Dir.path));
id_Post = cell(1,length(Dir.path));
id_Cond = cell(1,length(Dir.path));
...
for i = 1:length(Dir.path)
    ...
    Session{i} = load([Dir.path{i}{1} 'behavResources.mat'], 'SessionEpoch', 'behavResources');
    load('ExpeInfo.mat');
    ...
    % whole session
    try 
        Session{i}.SessionEpoch.BaselineSleep = and(Session{i}.SessionEpoch.BaselineSleep, Sleep{i}.SWSEpoch);
    catch
        Session{i}.SessionEpoch.BaselineSleep = and(Session{i}.SessionEpoch.PreSleep, Sleep{i}.SWSEpoch);
    end
    Session{i}.SessionEpoch.PostSleep = and(Session{i}.SessionEpoch.PostSleep, Sleep{i}.SWSEpoch);
    
    % Restrict sleeps to first 15 min
    Session{i}.SessionEpoch.BaselineSleep_first = RestrictToTime(and(Session{i}.SessionEpoch.BaselineSleep, Sleep{i}.SWSEpoch),...
        dur_parts*60*1e4);
    ...
    [Pre_split res] = SplitIntervals(and(Session{i}.SessionEpoch.BaselineSleep, Sleep{i}.SWSEpoch),dur_parts*60*1e4);
    [post_split res] = SplitIntervals(and(Session{i}.SessionEpoch.PostSleep, Sleep{i}.SWSEpoch),dur_parts*60*1e4);
    Session{i}.SessionEpoch.BaselineSleep_last = and(Pre_split{length(Pre_split)}, Sleep{i}.SWSEpoch);        
    ...
    Session{i}.SessionEpoch.BaselineSleep_bin = SplitIntervals(and(Session{i}.SessionEpoch.BaselineSleep, Sleep{i}.SWSEpoch),dur_small*60*1e4);
    ...
    %----------------- DENSITY -------------------
    %whole session
    PreRipples{i}=Restrict(ripplesPeak{i},Session{i}.SessionEpoch.BaselineSleep);
    ...
    TimePre{i} = and(Session{i}.SessionEpoch.BaselineSleep, Sleep{i}.SWSEpoch);
    ...
    PreRipples_first{i}=Restrict(ripplesPeak{i},Session{i}.SessionEpoch.BaselineSleep_first);
    ...
    % Normalize to the duration of sleep
    TimePre_first{i} = Session{i}.SessionEpoch.BaselineSleep_first;
    ...
    PreRipples_last{i}=Restrict(ripplesPeak{i},Session{i}.SessionEpoch.BaselineSleep_last);
    ...
    % Normalize to the duration of sleep
    TimePre_last{i} = Session{i}.SessionEpoch.BaselineSleep_last;
    ...
    % Ripples dynamics
    for ibin=1:length(Session{i}.SessionEpoch.BaselineSleep_bin)
        PreRipples_bin{i,ibin}=Restrict(ripplesPeak{i},Session{i}.SessionEpoch.BaselineSleep_bin{ibin});
    ...
    PreRipplesBeg{i}=Restrict(ripplesBeg{i},and(Session{i}.SessionEpoch.BaselineSleep, Sleep{i}.SWSEpoch));
    ...
    PreRipplesEnd{i}=Restrict(ripplesEnd{i},and(Session{i}.SessionEpoch.BaselineSleep, Sleep{i}.SWSEpoch));
    ...
    %Dynamics
    for ibin=1:length(Session{i}.SessionEpoch.BaselineSleep_bin)
        PreRipplesBeg_bin{i,ibin}=Restrict(ripplesBeg{i},Session{i}.SessionEpoch.BaselineSleep_bin{ibin});
        PreRipplesEnd_bin{i,ibin}=Restrict(ripplesEnd{i},Session{i}.SessionEpoch.BaselineSleep_bin{ibin});
    ...
    PreRipplesAmp{i}=Restrict(ripplesAmp{i},and(Session{i}.SessionEpoch.BaselineSleep, Sleep{i}.SWSEpoch));
    ...
    % Dynamics
    for ibin=1:length(Session{i}.SessionEpoch.BaselineSleep_bin)
        PreRipplesAmp_bin{i,ibin}=Restrict(ripplesAmp{i},Session{i}.SessionEpoch.BaselineSleep_bin{ibin});
...
figure('Color',[1 1 1], 'rend','painters','pos',[1 1 1800 400],'Name', supertit, 'NumberTitle','off')
    ...
    subplot(131)
        ...
        set(her, 'LineWidth', 2);
        ylabel('Ripples/s');
        title('Ripples density - complete sessions (NREM)', 'FontSize', 18);
    ...
    subplot(132)
        ...
        set(her, 'LineWidth', 2);
        ylabel('Ripples/s');
        title(['Ripples density in first ' num2str(dur_parts) ' min of NREM'], 'FontSize', 18);
    ...
    subplot(133)
        ...
        set(her, 'LineWidth', 2);
        ylabel('Ripples/s');
        title(['Ripples density in last ' num2str(dur_parts) ' min of NREM'], 'FontSize', 18);
...
figure('Color',[1 1 1], 'rend','painters','pos',[1 1 1800 400],'Name', supertit, 'NumberTitle','off')
    ...
    %% Save figure
    if sav
        print([dir_out 'ripples_details'], '-dpng', '-r600');
    end
