...
pathfig = [dropbox 'DataSL/StimMFBWake/Place Cells/']; 
%set folders
...
for j=1:length(Dir.path)
    ...
    beh = load('behavResources.mat','SessionEpoch','Vtsd','AlignedXtsd','AlignedYtsd');
    
    % Split the exploration in two
    try
        st = Start(beh.SessionEpoch.Hab2);
        en = End(beh.SessionEpoch.Hab2);
    catch 
    ...
    % Create BaselineExplo Epoch 
    try
        BigMazeEpoch = or(beh.SessionEpoch.Hab2,beh.SessionEpoch.TestPre1);
    catch
    ...
    AfterConditioningEpoch = or(AfterConditioningEpoch,beh.SessionEpoch.Extinct);
    
    % Locomotion threshold
    LocomotionEpoch = thresholdIntervals(tsd(Range(beh.Vtsd),movmedian(Data(beh.Vtsd),5)),speed_thresh,'Direction','Above'); % smoothing = 5
    
    % Get resulting epochs
    BigMazeMovingEpoch = and(LocomotionEpoch, BigMazeEpoch); 
    try
        UMazeMovingEpoch = and(beh.SessionEpoch.Hab2, LocomotionEpoch);
    catch
    ...
    if isfield(spikes.PlaceCells,'idx')
        for i=1:length(spikes.PlaceCells.idx)
            ...
            try % this try-catch syntax avoids spitting out an error in a situation when there are too less spikes for analysis on one half of the recording
                [map1, mapNS, stats{i}, px_within{1,i}, py_within{1,i}, FR{i}, xB, yB] = ...
                    PlaceField_DB(Restrict(spikes.S{spikes.PlaceCells.idx(i)},MovingHab1Half),... % 1st half
                    Restrict(beh.AlignedXtsd, MovingHab1Half),...
                    Restrict(beh.AlignedYtsd, MovingHab1Half), 'smoothing', 2.5, 'size', 50, 'plotresults',0, 'plotpoisson', 0);
                [map2, mapNS, stats{i}, px_within{2,i}, py_within{2,i}, FR{i}, xB, yB]  = ...
                    PlaceField_DB(Restrict(spikes.S{spikes.PlaceCells.idx(i)},MovingHab2Half),... % 2nd half
                    Restrict(beh.AlignedXtsd, MovingHab2Half),...
                    Restrict(beh.AlignedYtsd, MovingHab2Half), 'smoothing', 2.5, 'size', 50, 'plotresults',0, 'plotpoisson', 0);
            catch
                map1 = [];
                map2 = [];
            end
            ...
            try % this try-catch syntax avoids spitting out an error in a situation when there are too less spikes for analysis on one half of the recording
                [map1, mapNS, stats{i}, px_between{1,i}, py_between{1,i}, FR{i}, xB, yB] = ...
                    PlaceField_DB(Restrict(spikes.S{spikes.PlaceCells.idx(i)},UMazeMovingEpoch),... % Exploration before aversive experience
                    Restrict(beh.AlignedXtsd, UMazeMovingEpoch),...
                    Restrict(beh.AlignedYtsd, UMazeMovingEpoch), 'smoothing', 2.5, 'size', 50, 'plotresults',0, 'plotpoisson', 0);
                [map2, mapNS, stats{i}, px_between{2,i}, py_between{2,i}, FR{i}, xB, yB]  = ...
                    PlaceField_DB(Restrict(spikes.S{spikes.PlaceCells.idx(i)},AfterConditioningMovingEpoch),... % Exploration after aversive experience
                    Restrict(beh.AlignedXtsd, AfterConditioningMovingEpoch),...
                    Restrict(beh.AlignedYtsd, AfterConditioningMovingEpoch), 'smoothing', 2.5, 'size', 50, 'plotresults', 0, 'plotpoisson', 0);
            catch
                map1 = [];
                map2 = [];
            end
            ...
            % save rate maps
            map_between{1,i} = map1;
            map_between{2,i} = map2;
            
            clear map1 map2
        end
    end

    % Figure : rate maps visual comparison side-by-side (TRAJECTORIES and
    % FR)
    for ipc=1:length(spikes.PlaceCells.idx)
        supertit = ['PC #' num2str(spikes.PlaceCells.idx(ipc)) ': PF stability'];
        figure2(1,'Color',[1 1 1], 'rend','painters','pos',[1 1 1000 800],'Name', supertit, 'NumberTitle','off')
            for imap=1:2
                subplot(2,2,imap)
%                         imagesc(rescale(map_within{imap,ipc}.rate))
                    plot(px_within{imap,ipc},py_within{imap,ipc},'r.')
                    set(gca,'xticklabel',{[]})
                    set(gca,'yticklabel',{[]})
                    hold on 
                    if imap == 1
                        plot(Data(Restrict(beh.AlignedXtsd, MovingHab1Half)),Data(Restrict(beh.AlignedYtsd, MovingHab1Half)),...
                            'Color',[0.8 0.8 0.8])
                    else
                        plot(Data(Restrict(beh.AlignedXtsd, MovingHab2Half)),Data(Restrict(beh.AlignedYtsd, MovingHab2Half)),...
                            'Color',[0.8 0.8 0.8])
                    end
                    title(['Within: part ' num2str(imap)])
                    set(gca,'xticklabel',{[]})
                    set(gca,'yticklabel',{[]})
                subplot(2,2,imap+2)
%                     imagesc(map_between{imap,ipc}.rate)
                    plot(px_between{imap,ipc},py_between{imap,ipc},'r.')
                    set(gca,'xticklabel',{[]})
                    set(gca,'yticklabel',{[]})
                    hold on 
                    if imap==1
                        plot(Data(Restrict(beh.AlignedXtsd, UMazeMovingEpoch)), ...
                            Data(Restrict(beh.AlignedYtsd, UMazeMovingEpoch)),...
                            'Color',[0.8 0.8 0.8])
                        title('Pre-tests')
                    else
                        plot(Data(Restrict(beh.AlignedXtsd, AfterConditioningMovingEpoch)), ...
                            Data(Restrict(beh.AlignedYtsd, AfterConditioningMovingEpoch)),...
                            'Color',[0.8 0.8 0.8])
                        title('Post-tests')
                    end
                    set(gca,'xticklabel',{[]})
        ...
        print([pathfig 'PC_Stability_traj' num2str(spikes.PlaceCells.idx(ipc))], '-dpng', '-r300');
    end
    ...
    for ipc=1:length(spikes.PlaceCells.idx)
        ...
        figure2(1,'Color',[1 1 1], 'rend','painters','pos',[1 1 1000 800],'Name', supertit, 'NumberTitle','off')
            for imap=1:2
                subplot(2,2,imap)
                    imagesc(rescale(map_within{imap,ipc}.rate)), axis xy
                    title(['Within: part ' num2str(imap)])
                    set(gca,'xticklabel',{[]})
                    set(gca,'yticklabel',{[]})
                subplot(2,2,imap+2)
                    imagesc(map_between{imap,ipc}.rate), axis xy
                    if imap==1
                        title('Pre-tests')
                    else
                        title('Post-tests')
        ...
        print([pathfig 'PC_Stability_ratemaps' num2str(spikes.PlaceCells.idx(ipc))], '-dpng', '-r300');        
    end
    clear spikes st en LocomotionEpoch MovingHab1Half MovingHab2Half AfterConditioningMovingEpoch BigMazeMovingEpoch UMazeMovingEpoch
...
for i=1:length(badcases)
    ...
    load('behavResources.mat','SessionEpoch','Vtsd','AlignedXtsd','AlignedYtsd');
    % Locomotion threshold
    LocomotionEpoch = thresholdIntervals(tsd(Range(Vtsd),movmedian(Data(Vtsd),5)),2.5,'Direction','Above'); % smoothing = 5   
    % Get resulting epochs
    ...
    map = PlaceField_DB(Restrict(S{badcases{i}(2)},UMazeMovingEpoch),... % 1st half
                    ...
                    Restrict(AlignedYtsd, UMazeMovingEpoch), 'smoothing', 2.5, 'size', 50, 'plotresults',0, 'plotpoisson', 0);
%     map = PlaceField_DB(Restrict(S{badcases{i}(2)},SessionEpoch.Hab),... % 1st half
%                     Restrict(AlignedXtsd, SessionEpoch.Hab),...
%                     Restrict(AlignedYtsd, SessionEpoch.Hab), 'smoothing', 2.5, 'size', 50, 'plotresults',1, 'plotpoisson', 1);
    figure
        imagesc(map.rate);
        colormap jet
        title([Dir.name{badcases{i}(1)} '  PC#' num2str(badcases{i}(2))]);
    
    clear S PlaceCells SessionEpoch Vtsd AlignedXtsd AlignedYtsd LocomotionEpoch UMazeMovingEpoch map
...
%% Plot barplot% Pl = {CorrStabAfterCondCell_all; CorrStabSameCell_all};
...
if sav
    print([pathfig 'PC_Stability_WithinVsBetween'], '-dpng', '-r300'); 
end
