...
pathfig = [dropbox '\DataSL\FirstExplo\New/Place Cells\Place Fields/']; % without dropbox path
%set folders
...
for j=1:length(Dir.path)
    ...
    st = Start(beh.SessionEpoch.Hab1);
    en = End(beh.SessionEpoch.Hab1);
    ...
    % After Conditioning
    AfterConditioningEpoch = beh.SessionEpoch.Hab2;
...
%     AfterConditioningEpoch = or(AfterConditioningEpoch,beh.SessionEpoch.TestPost4);
    ...
    UMazeMovingEpoch = and(beh.SessionEpoch.Hab1, LocomotionEpoch);
...
for i=1:length(badcases)
    ...
    % Get resulting epochs
    UMazeMovingEpoch = and(SessionEpoch.Hab2, LocomotionEpoch);
...
if sav
    saveas(fb,[pathfig figbar '.fig']);
    saveFigure(fb,figbar,[pathfig]);
end
