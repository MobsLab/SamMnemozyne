...
% AlignedYtsd and CleanVtsd from behavResources.mat.
% 
...
pathfig = [dropbox 'DataSL/StimMFBWake/Place Cells/Place Fields/']; 
%set folders
...
for i=1:length(Dir.path)
    ...
    if EpochLong
        try
            try
                UMazeEpoch{i} = or(behav{i}.SessionEpoch.Hab2,behav{i}.SessionEpoch.TestPre1);
            catch
...
for i=1:length(Dir.path)
    ...
    for j=1:length(spikes{i}.S)
        ...
        if lEpoch > lThresh && FRinEpoch > FRthresh
            try
                [map{cntAll}, ~, stats_temp{cntAll}, ~, ~, FR{cntAll}]=PlaceField_DB(spikes{i}.S{j},behav{i}.AlignedXtsd, behav{i}.AlignedYtsd,...
...
figure2(1,'Color',[1 1 1], 'rend','painters','pos',[1 1 1000 800],'Name', supertit, 'NumberTitle','off')
    ...
    plot(ShockZoneMap(:,1),ShockZoneMap(:,2),'g','LineWidth',4)
    ...
    if savfig
        print([pathfig 'AllPlaceFields_Cur'], '-dpng', '-r300');
    end
