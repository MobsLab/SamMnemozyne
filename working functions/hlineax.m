function hhh=hlineax(ax,y,in1,in2)
% function h=hlineax(ax, y, linetype, label)
% 
...
%%
%  Updated by S. Laventure for use with specified axes
%%
...
if length(y)>1  % vector input
    for I=1:length(y)
        ...
        h(I)=hline(ax,y(I),linetype,label);
    end
else
    switch nargin
    case 1
...
%     h=plot(x,[y y],linetype);
    ...
    if ~isempty(label)
        ...
        yunit=(y-yy(1))/yrange;
        if yunit<0.2
            text(ax,x(1)+0.02*(x(2)-x(1)),y+0.02*yrange,label,'color',get(h,'color'))
        else
            text(ax,x(1)+0.02*(x(2)-x(1)),y-0.02*yrange,label,'color',get(h,'color'))
        end
    end
