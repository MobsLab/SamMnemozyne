...
% Get directories
Dir = PathForExperimentsERC_SL('CalibMFB');
...
%941
sessN{1} = {'mfb2';'mfb4';'mfb14'};    
%016
sessN{2} = {'mfb1','mfb4','mfb11'};
%936
% sessN{3} = {'mfb1','dual3','dual5'};
...
save([dir_out 'calibmfb.mat'],'ratepre_all_mean','ratepre_all_std','ratepost_all_mean','ratepost_all_std',...
    'volt','nbtrials','nbstim');
...
%     miny = min([min(min(ratepre_mean(imouse,:,:)-ratepre_std(imouse,:,:))) min(min(ratepost_mean(imouse,:,:)-ratepost_std(imouse,:,:)))])*0.99;
    ...
    figure2(1,'Color',[1 1 1], 'rend','painters','pos',[1 1 900 600],'Name', supertit, 'NumberTitle','off')
        ...
        for itrial=1:stp:ntrial
            if nbstim(itrial)>0  % SEE DUAL SCRIPT FOR PLOT WITH SHADES (on laptop)
...
%                 hold on
%                 h(itrial)=plot([ttime*1E4+postbuff:(ttime*2+stimdelay)*1e4-5000], squeeze(squeeze(ratepost_all_mean{1,itrial}(postbuff:end-5000))),... 
...
%                 hold on
        ...
        title('Heart rate locked to stim during MFB calibration')

        hold off
        % Save figure
        if sav
            if smoo
                print([dir_out 'MFBCalib_HR_stimlock_LowMedHI_smooth'], '-dpng', '-r600');
            else
                print([dir_out 'MFBCalib_HR_stimlock_LowMedHI'], '-dpng', '-r600');
            end
        end
...
%                         squeeze(squeeze(ratepost_std(imouse,itrial,postbuff:end-5000))),clrs_letter{itrial},1);
        ...
        if sav
            if smoo
                print([dir_out 'MFBCalib_HR_stimlock_smooth'], '-dpng', '-r600');
            else
