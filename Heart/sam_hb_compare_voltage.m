function sam_hb_compare_voltage(varargin)

%==========================================================================
% Details: compare hb during calibrations . Needs to input manually in the
...
% Get directories
Dir = PathForExperimentsERC_SL_home('CalibMFB');
...
%   |-------------------------------------------------------------------------------------------------------------------
% Get directories
sessN = {'mfb1';'mfb2';'mfb3';'mfb4';'mfb5';'mfb6';'mfb8';'mfb10';'mfb11';'mfb12';'mfb13';'mfb14'};  % session 7 and 9 are both 4V as welll as session 8 (chose just one)

% Voltage
volt = {'0V','1V','1.5V','2V','2.5V','3.5V','4V','4.5V','5V','5.5V','6V','6.5V'};
%-----------------------------------------------------------------------------------------------------------------------

% set nbr of mice and trials
nbmice = length(Dir.path);
nbtrials = length(volt);
...
sav = 1;
ntrial = length(sessN);  %assuming that all pre, post and cond have the same number of trials 
...
for imouse=1:nbmice
    ...
    for itrial=1:ntrial
        [idx stimes{imouse,itrial}] = RestrictSession(Dir,sessN{itrial});
    end
...
for imouse=1:nbmice
    for itrial=1:ntrial
        if ~restricted
            hb{imouse,itrial} = Restrict(Restrict(ekg{imouse}.EKG.HBRate,stimes{imouse,itrial}{1}{1}),ekg{imouse}.EKG.GoodEpoch);
        else
            hb{imouse,itrial} = RestrictThreshold(Restrict(ekg{imouse}.EKG.HBRate,stimes{imouse}{1}{1}),thresh);
        end
        ...
        tstim = Start(and(behav{imouse}.StimEpoch, stimes{imouse,itrial}{1}{1}));
        ...
        if length(tstim)
            ...
            warning(['Mouse ' num2str(nmice(imouse)) ' has no valid stim during ' volt{imouse,itrial} ' trial']);
...
%     % Overall means
%     if nbmice > 1
%         ratepre_all_mean{imouse} = squeeze(nanmean(nanmean(ratepre_mean(imouse,:,:),2)));
%         ratepre_all_std{imouse} = squeeze(nanmean(nanstd(ratepre_mean(imouse,:,:),2)));
%         ratepost_all_mean{imouse} = squeeze(nanmean(nanmean(ratepost_mean(imouse,:,:),2)));
%         ratepost_all_std{imouse} = squeeze(nanmean(nanstd(ratepost_mean(imouse,:,:),2)));
%     else
%         ratepre_all_mean{imouse} = squeeze(ratepre_mean(imouse,:,:));
%         ratepre_all_std{imouse} = squeeze(ratepre_std(imouse,:,:));
%         ratepost_all_mean{imouse} = squeeze(ratepost_mean(imouse,:,:));
%         ratepost_all_std{imouse} = squeeze(ratepost_std(imouse,:,:));
%     end
...
%     % smooth data
%     if smoo
%         ratepre_all_mean{imouse} = smooth(ratepre_all_mean{imouse},sm);
%         ratepre_all_std{imouse} = smooth(ratepre_all_std{imouse},sm);    
%         ratepost_all_mean{imouse} = smooth(ratepost_all_mean{imouse},sm);
%         ratepost_all_std{imouse} = smooth(ratepost_all_std{imouse},sm);
%     end
%     
%     % standardize data
...
for imouse=1:nbmice
    % set limits
    maxy = max([max(max(ratepre_mean(imouse,:,:)+ratepre_std(imouse,:,:)))  max(max(ratepost_mean(imouse,:,:)+ratepost_std(imouse,:,:)))])*1.05;
    miny = min([min(min(ratepre_mean(imouse,:,:)-ratepre_std(imouse,:,:))) min(min(ratepost_mean(imouse,:,:)-ratepost_std(imouse,:,:)))])*0.95;
    % set legend text
    idel=1;
    for i=1:nbtrials
        if nbstim(i)>0
    ...
    %                         squeeze(squeeze(ratepost_std(imouse,itrial,postbuff:end-5000))),clrs_letter{itrial},1);
        ...
        % Save figure
        if sav
            print([dir_out 'MFBCalib_HR_stimlock'], '-dpng', '-r600');
        end
end
%--------------------------------------------------------------------------
